...
import {
  ...
  exactTableCurrency,
  titleCase,
} from "./budget-office-allocation-table";
...
export function BudgetProgramTable({
  ...
  return (
    <section id="programs" className="my-12 scroll-mt-[240px]">
      <div className="overflow-x-auto bg-[var(--paper)]">
        <div className="min-w-[1040px]">
          {visiblePrograms.map((program) => {
            const allocationTotal =
              program.personnel_services +
                program.mooe +
                program.capital_outlays || program.total;
          ...
          {visiblePrograms.length === 0 ? (
            <div className="px-6 py-10 text-sm leading-6 text-[var(--ink-3)]">
        ...
        {objectDistributions.length > 0 ? (
                  ...
                  {distribution.items.map((item, itemIndex) => (
                    <div
                      key={[
                        ...
                        item.name,
                        item.amount,
                        item.sourcePage ?? "source",
                        item.objectGroup ?? distribution.label,
                        itemIndex,
                      ].join("-")}
                      className="grid grid-cols-[minmax(20rem,1fr)_12rem_minmax(16rem,35%)] items-center gap-5 text-sm"
                    >
                      <p className="font-semibold text-[var(--ink)]">
                        {item.name}
                      </p>
                      <p className="num shrink-0 text-right text-[var(--ink-2)]">
          ...
          <div className="mt-5 border border-[var(--rule)] bg-[var(--paper)] p-4">
            <p className="text-sm leading-6 text-[var(--ink-3)]">
              The comprehensive source does not itemize this program down to
              object-level rows. The PS, MOOE, and CO distribution is shown from
              the program amount columns.
            </p>
          </div>
        )}
      </div>
